    ...
    def get_data(self, scene):
        ...
        for name, material in scene.materials.items():
            ...
            for i in range(0, len(vertices[name]), 6):
                data.extend([vertices[name][i], vertices[name][i + 1], vertices[name][i + 2]])
                data.extend(vertices[name][i + 3:i + 6])
                data.extend(material.diffuse[:3])
                data.extend(material.ambient[:3])
